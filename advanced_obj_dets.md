...
- 多任务
  ...
  - 训练和推理方式:训练时将proposal与GT重合的部分像素作为真值(将proposal resize到全卷积输出的尺寸,大于一定阈值作为正样本)。类似地,推理时将全卷积输出对应通道取出后resize到输出框尺寸，然后用阈值过滤。为了减少计算量,mask支路相对目标检测支路针对更少量的proposal进行计算.
...
- 改善多尺度目标检测性能
  ...
  - TridentNet
    ...
    - scale aware training: 不同尺度分支的预测上,设置sqrt(wh)区间[l,h],当anchor和proposal位于区间内时,分配至这一支。推理时,首先过滤掉位于区间外的结果，然后多支一起做NMS.(在coco上根据s,m,l来设置,并有一定重合)
...
- 改造anchor
  ...
  - 训练方式:在普通的检测器loss额外再加两个anchor位置和形状loss
    ...
    - anchor形状loss: 首先计算在该位置上能产生最大IOU的宽高.因连续解析计算困难，实际上采用的是采样几个点(用的是retinaNet的anchor形状参数)，代进去计算的argmax.然后使用iou bounded loss: L1(1-min(w/w_g,w_g/w))+L1(1-min(h/h_gm,h_g/h))
